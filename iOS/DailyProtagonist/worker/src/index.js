 ...
 * Handles face photo storage and AI image generation
 */
...
async function handleDailyImage(request, env) {
  try {
    ...
    const cacheKey = `image:${user_id}:${today}`;

    // Check if we already have a cached image for today
    const cachedImage = await env.IMAGE_CACHE.get(cacheKey, { type: 'json' });
    if (cachedImage) {
      console.log(`Cache hit for ${user_id} on ${today}`);
      return new Response(JSON.stringify({
        date: today,
        cached: true,
        image_base64: cachedImage.image_base64,
        prompt_used: cachedImage.prompt_used
      }), {
        headers: {
    ...
    // Get stored face photo (if exists)
    let facePhoto = null;
    if (!face_base64) {
      facePhoto = await env.FACE_STORE.get(`face:${user_id}`);
      if (!facePhoto) {
        return new Response(JSON.stringify({
          error: 'No face photo found. Please upload a face photo first.',
          code: 'NO_FACE_PHOTO'
        }), {
          status: 400,
          headers: { 'Content-Type': 'application/json' }
      ...
      // First time - store the face photo
      await env.FACE_STORE.put(`face:${user_id}`, face_base64);
      console.log(`Stored face photo for ${user_id}`);
    }

    // Generate image using AI API
    const imageResult = await generateImage(story_text, face_base64 || facePhoto, env);

    // Cache the result
    await env.IMAGE_CACHE.put(cacheKey, JSON.stringify(imageResult), {
...
/**
 * Generate image using AI API
 * Configure your AI provider below (OpenAI, Stability AI, Replicate, etc.)
 *
...
async function generateImage(storyText, faceBase64, env) {
  ...
  if (env.OPENAI_API_KEY) {
    ...
    return await generateWithDALL-E(storyText, faceBase64, env);
  }
  ...
  throw new Error('No AI API key configured. Please set REPLICATE_API_KEY (recommended for face-to-image), OPENAI_API_KEY, or STABILITY_API_KEY in your Worker environment variables.');
}

/**
 * Generate image using OpenAI DALL-E
 */
async function generateWithDALL-E(storyText, faceBase64, env) {
